...
$pageTitle = 'Add New Admin | College Management System';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        redirect('/views/admin/add-admin.php');
    }
    ...
    if (!empty($errors)) {
        ...
        try {
            ...
            $insertStmt = $db->prepare("INSERT INTO admins (name, email, phone, password_hash, role, status) VALUES (?, ?, ?, ?, ?, ?)");
            $insertStmt->execute([$name, $email, $phone ?: null, $hash, $role, $status]);
            $newAdminId = (int)$db->lastInsertId();

            // Insert permissions
            if (!empty($selectedPermissions)) {
                // Always ensure dashboard access
                if (!in_array('1', $selectedPermissions)) {
                ...
                foreach ($selectedPermissions as $moduleId) {
                    ...
                    if ($moduleId > 0) {
                        $permInsert->execute([$newAdminId, $moduleId]);
                    }
                }
            }
            ...
            set_flash_message('Failed to create admin account. Please try again.', 'error');
        }
    }
}
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
                    <i data-lucide="user-plus" class="w-7 h-7 text-indigo-600 dark:text-indigo-400"></i>
                    Add New Admin
                </h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Create a new administrator account and assign permissions.</p>
            </div>
        ...
        <form method="POST" class="space-y-6">
                ...
                <div class="p-6 space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="<?= htmlspecialchars($_POST['name'] ?? '') ?>" required class="w-full px-4 py-2.5 text-sm border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors" placeholder="e.g., Dr. John Doe">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <select id="role" name="role" required class="w-full px-4 py-2.5 text-sm border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors">
                                ...
                                <?php foreach ($availableRoles as $r): ?>
                                    <option value="<?= htmlspecialchars($r) ?>" <?= ($_POST['role'] ?? '') === $r ? 'selected' : '' ?>><?= htmlspecialchars($r) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="password" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Password <span class="text-rose-500">*</span></label>
                            <input type="password" id="password" name="password" required minlength="8" class="w-full px-4 py-2.5 text-sm border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors" placeholder="Min. 8 characters">
                        </div>
                        <div>
                            <label for="confirm_password" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Confirm Password <span class="text-rose-500">*</span></label>
                            <input type="password" id="confirm_password" name="confirm_password" required minlength="8" class="w-full px-4 py-2.5 text-sm border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors" placeholder="Re-enter password">
                        </div>
                    </div>
                    <div>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    <?php foreach ($modules as $mod): ?>
                                        <?php $isDashboard = ($mod['module_key'] === 'dashboard'); ?>
                                        <?php $modId = $mod['id'] ?? $mod['module_id'] ?? $mod['ID'] ?? 0; ?>
                                        <label class="flex items-center gap-3 p-3 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors cursor-pointer <?= $isDashboard ? 'opacity-70' : '' ?>">
                                            <input type="checkbox" name="permissions[]" value="<?= $modId ?>" 
                                                ...
                                                <?= in_array((string)$modId, $_POST['permissions'] ?? []) ? 'checked' : '' ?>>
                                            <?php if ($isDashboard): ?>
                                                <!-- Hidden input to ensure dashboard is always submitted -->
            ...
            <div class="flex items-center justify-end gap-3">
                ...
                <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold transition-all shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 hover:-translate-y-0.5">
                    <i data-lucide="user-plus" class="w-4 h-4"></i> Create Admin
                </button>
            </div>
        </form>
    </div>
