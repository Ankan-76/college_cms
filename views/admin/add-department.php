<?php
// views/admin/add-department.php
require_once __DIR__ . '/../../config/app.php';
require_once __DIR__ . '/../../includes/auth_middleware.php';
require_once __DIR__ . '/../../config/database.php';

require_role('ADMIN');
$pageTitle = 'Add Department | College Management System';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($dept_code) || empty($dept_name)) {
        ...
        if ($stmt->fetch()) {
            ...
            try {
                $stmt = $db->prepare("INSERT INTO departments (dept_code, dept_name, total_semesters) VALUES (?, ?, ?)");
                $stmt->execute([$dept_code, $dept_name, $total_semesters]);
                header('Location: departments.php');
                exit;
            } catch (PDOException $e) {
                $error = 'An error occurred while adding the department.';
            }
        }
    }
}
        ...
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
                    <i data-lucide="plus-circle" class="w-6 h-6 text-indigo-500"></i> Add Department
                </h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Create a new academic department.</p>
            </div>
            ...
            <form method="POST" action="">
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="number" name="total_semesters" id="total_semesters" required min="1" max="12"
                            class="w-full rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-primary focus:border-primary shadow-sm sm:text-sm px-4 py-3 border outline-none transition-shadow"
                            placeholder="e.g. 8"
                            value="<?= htmlspecialchars($_POST['total_semesters'] ?? '8') ?>">
                    </div>
                </div>
                <div class="mt-8 flex justify-end">
                    <button type="submit" class="inline-flex items-center gap-2 bg-primary hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-sm focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-slate-900 hover:-translate-y-0.5">
                        <i data-lucide="save" class="w-4 h-4"></i> Save Department
                    </button>
                </div>
            </form>
        </div>
