<?php
// views/admin/add-notice.php
require_once __DIR__ . '/../../config/app.php';
require_once __DIR__ . '/../../includes/auth_middleware.php';
require_once __DIR__ . '/../../config/database.php';

require_role('ADMIN');
$pageTitle = 'Add Notice | College Management System';
...
function generateSlug($title) {
    return strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $title), '-')) . '-' . substr(md5(uniqid()), 0, 5);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $attachment_url = null; // Basic version without file upload
    ...
    if (empty($title) || empty($content)) {
        ...
        try {
            ...
            $stmt = $db->prepare("INSERT INTO notices (title, slug, content, target_role, attachment_url, created_by, is_pinned) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$title, $slug, $content, $target_role, $attachment_url, $_SESSION['user_id'], $is_pinned]);
            header('Location: notices.php');
            exit;
        } catch (PDOException $e) {
            $error = 'Error publishing notice.';
        }
    }
}
        ...
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
                    <i data-lucide="bell-plus" class="w-6 h-6 text-indigo-500"></i> Add Notice
                </h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Publish an official announcement.</p>
            </div>
            ...
            <form method="POST" action="">
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <select name="target_role" class="w-full rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-primary focus:border-primary shadow-sm sm:text-sm px-4 py-3 border outline-none transition-shadow">
                                <option value="ALL" <?= (isset($_POST['target_role']) && $_POST['target_role'] == 'ALL') ? 'selected' : '' ?>>All Roles</option>
                                <option value="FACULTY" <?= (isset($_POST['target_role']) && $_POST['target_role'] == 'FACULTY') ? 'selected' : '' ?>>Faculty Only</option>
                                <option value="STUDENT" <?= (isset($_POST['target_role']) && $_POST['target_role'] == 'STUDENT') ? 'selected' : '' ?>>Students Only</option>
                            </select>
                        </div>
                        <div class="flex items-center pt-8">
                            <label class="flex items-center gap-3 cursor-pointer">
                                <div class="relative flex items-center">
                                    <input type="checkbox" name="is_pinned" class="peer sr-only" <?= isset($_POST['is_pinned']) ? 'checked' : '' ?>>
                    ...
                    <div>
                        ...
                        <textarea name="content" required rows="6"
                            class="w-full rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-primary focus:border-primary shadow-sm sm:text-sm px-4 py-3 border outline-none transition-shadow"><?= htmlspecialchars($_POST['content'] ?? '') ?></textarea>
                    </div>
                </div>
                ...
                <div class="mt-8 flex justify-end">
                    <button type="submit" class="inline-flex items-center gap-2 bg-primary hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-sm focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-slate-900 hover:-translate-y-0.5">
                        <i data-lucide="send" class="w-4 h-4"></i> Publish Notice
                    </button>
                </div>
            </form>
        </div>
