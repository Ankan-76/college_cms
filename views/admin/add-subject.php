...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$department_id || !$semester_id || !$course_code || !$course_name || !$credits) {
        ...
        if ($stmt->fetch()) {
            ...
            try {
                $stmt = $db->prepare("INSERT INTO courses (department_id, semester_id, course_code, course_name, credits) VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([$department_id, $semester_id, $course_code, $course_name, $credits]);
        ...
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
                    <i data-lucide="plus-circle" class="w-6 h-6 text-indigo-500"></i> Add Subject
                </h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Create a new academic subject.</p>
            </div>
            ...
            <form method="POST" action="">
                ...
                <div class="mt-8 flex justify-end">
                    <button type="submit" class="inline-flex items-center gap-2 bg-primary hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-sm focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-slate-900 hover:-translate-y-0.5">
                        <i data-lucide="check" class="w-4 h-4"></i> Create Subject
                    </button>
                </div>
            </form>
        </div>
