...
$pageTitle = 'Add Timetable | College Management System';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$department_id || !$semester_id || !$course_id || !$faculty_id || !$day_of_week || !$start_time || !$end_time) {
        ...
        ");
        $conflictStmt->execute([$faculty_id, $day_of_week, $start_time, $start_time, $end_time, $end_time]);
        ...
        if ($conflictStmt->fetch()) {
            ...
            $classConflictStmt = $db->prepare("
                SELECT id FROM timetables 
                WHERE department_id = ? AND semester_id = ? AND day_of_week = ? 
                AND ((start_time <= ? AND end_time > ?) OR (start_time < ? AND end_time >= ?))
            ");
            $classConflictStmt->execute([$department_id, $semester_id, $day_of_week, $start_time, $start_time, $end_time, $end_time]);
            ...
            if ($classConflictStmt->fetch()) {
                ...
                try {
                    $stmt = $db->prepare("
                        INSERT INTO timetables (department_id, semester_id, course_id, faculty_id, day_of_week, start_time, end_time, room_number)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    ");
                    $stmt->execute([$department_id, $semester_id, $course_id, $faculty_id, $day_of_week, $start_time, $end_time, $room_number]);
                    header('Location: timetables.php?department_id=' . $department_id . '&semester_id=' . $semester_id);
                    exit;
                } catch (PDOException $e) {
                    $error = 'Database error: Unable to add timetable entry.';
                }
            }
        }
    }
        ...
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
                    <i data-lucide="plus-circle" class="w-6 h-6 text-indigo-500"></i> Add Timetable
                </h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Schedule a new class session.</p>
            </div>
            <a href="timetables.php" class="text-sm font-semibold text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 flex items-center gap-1">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden p-6 sm:p-8">
            ...
            <form method="POST" action="">
                            ...
                            <div>
                                ...
                                <select name="day_of_week" required class="w-full rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-primary focus:border-primary shadow-sm sm:text-sm px-4 py-3 border outline-none transition-shadow">
                                    ...
                                    <?php foreach($days as $day): ?>
                                        <option value="<?= $day ?>" <?= (isset($_POST['day_of_week']) && $_POST['day_of_week'] == $day) ? 'selected' : '' ?>>
                                            <?= $day ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                ...
                <div class="mt-8 flex justify-end border-t border-slate-200 dark:border-slate-700 pt-6">
                    <button type="submit" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl text-sm font-bold transition-all shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 hover:-translate-y-0.5">
                        <i data-lucide="calendar-plus" class="w-5 h-5"></i> Save Schedule
                    </button>
                </div>
            </form>
        </div>
