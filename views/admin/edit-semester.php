...
$pageTitle = 'Edit Semester | College Management System';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$semester_number || empty($academic_year)) {
        ...
        $stmt = $db->prepare("SELECT id FROM semesters WHERE semester_number = ? AND academic_year = ? AND id != ?");
        $stmt->execute([$semester_number, $academic_year, $id]);
        if ($stmt->fetch()) {
            $error = 'This semester already exists for the given academic year.';
        } else {
            try {
                $stmt = $db->prepare("UPDATE semesters SET semester_number = ?, academic_year = ?, start_date = ?, end_date = ?, status = ? WHERE id = ?");
                $stmt->execute([$semester_number, $academic_year, $start_date, $end_date, $status, $id]);
                header('Location: semesters.php');
                exit;
        ...
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
                    <i data-lucide="edit" class="w-6 h-6 text-indigo-500"></i> Edit Semester
                </h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Update academic term details.</p>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="number" name="semester_number" required min="1" max="12"
                                ...
                                value="<?= htmlspecialchars($semester['semester_number']) ?>">
                        </div>
                        <div>
