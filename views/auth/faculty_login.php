...
require_guest();
$pageTitle = 'Faculty Login | Greenfield University';
?>
<!DOCTYPE html>
...
<head>
    ...
    <script>
        tailwind.config = {
            darkMode: 'class',
            theme: { extend: { colors: { primary: '#059669' }, fontFamily: { sans: ['Inter', 'sans-serif'] } } }
        }
    </script>
...
<body class="bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 min-h-screen flex items-center justify-center p-4 font-sans relative overflow-hidden transition-colors">
    ...
    <div class="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <div class="absolute -top-40 -right-40 w-96 h-96 bg-emerald-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 dark:opacity-15 animate-blob"></div>
        <div class="absolute -bottom-40 -left-40 w-96 h-96 bg-teal-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 dark:opacity-15 animate-blob" style="animation-delay: 2s;"></div>
    </div>
    ...
    <div class="max-w-md lg:max-w-4xl xl:max-w-5xl w-full bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden animate-fade-in border border-slate-200/50 dark:border-slate-700/50 z-10 relative flex flex-col lg:flex-row">
        ...
        <div class="w-full lg:w-[45%] flex flex-col justify-between">
            ...
            <div class="text-center mb-10">
                <div class="w-16 h-16 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-full flex items-center justify-center mx-auto mb-4 border border-emerald-100 dark:border-emerald-800 shadow-sm transition-transform hover:scale-105 duration-300">
                    <i data-lucide="briefcase" class="w-8 h-8"></i>
                </div>
                <h2 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Faculty Portal</h2>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">Sign in to manage your courses and attendance</p>
            </div>
            ...
            <form action="<?= htmlspecialchars(BASE_URL) ?>/index.php?action=login" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="email" name="email" id="email" required class="block w-full pl-12 pr-4 rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm focus:border-emerald-500 focus:ring-emerald-500 focus:ring-2 sm:text-sm py-3 outline-none transition-all placeholder-slate-400 dark:placeholder-slate-500" placeholder="faculty@greenfield.edu">
                    </div>
                </div>
            ...
            <a href="<?= htmlspecialchars(BASE_URL) ?>/" class="text-xs text-emerald-600 dark:text-emerald-400 font-semibold hover:underline flex items-center gap-1">
