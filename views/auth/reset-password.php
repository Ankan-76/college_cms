...
if (!isset($_SESSION['password_reset']) || empty($_SESSION['password_reset']['verified'])) {
    set_flash_message('Please verify your OTP first.', 'error');
    redirect('/views/auth/verify-otp.php');
}

$pageTitle = 'Create New Password | Greenfield University';
?>
<!DOCTYPE html>
    ...
    <div class="max-w-md w-full bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden animate-fade-in border border-slate-200/50 dark:border-slate-700/50 z-10 relative">
        <div class="p-8 sm:p-10">
            <div class="text-center mb-10">
                ...
                <h2 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">New Password</h2>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">Create a strong new password for your account.</p>
            </div>
            
            <form action="<?= htmlspecialchars(BASE_URL) ?>/index.php?action=reset-password" method="POST" class="space-y-6">
                <?= csrf_field() ?>
                
                <div>
                    <label for="new_password" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">New Password</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            <i data-lucide="lock" class="w-5 h-5 text-slate-400"></i>
                        </div>
                        <input type="password" name="new_password" id="new_password" required minlength="8" class="block w-full pl-12 pr-12 rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 focus:ring-2 sm:text-sm py-3 outline-none transition-all placeholder-slate-400 dark:placeholder-slate-500" placeholder="••••••••">
                        <button type="button" onclick="togglePassword('new_password', 'eye-icon-new')" class="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 focus:outline-none">
                            <i id="eye-icon-new" data-lucide="eye" class="w-5 h-5"></i>
                        </button>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <button type="button" onclick="togglePassword('confirm_password', 'eye-icon-confirm')" class="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 focus:outline-none">
        ...
        <div class="px-8 py-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-700/50 text-center flex items-center justify-between">
            <a href="<?= htmlspecialchars(BASE_URL) ?>/" class="text-xs text-indigo-600 dark:text-indigo-400 font-semibold hover:underline flex items-center gap-1">
