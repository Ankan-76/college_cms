...
if (!isset($_SESSION['password_reset'])) {
    set_flash_message('Please enter your email to reset your password.', 'error');
    redirect('/views/auth/forgot-password.php');
}

$pageTitle = 'Verify OTP | Greenfield University';
?>
<!DOCTYPE html>
    ...
    <div class="max-w-md w-full bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden animate-fade-in border border-slate-200/50 dark:border-slate-700/50 z-10 relative">
        <div class="p-8 sm:p-10">
            <div class="text-center mb-10">
                ...
                <h2 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Verify OTP</h2>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">Enter the 4-digit code sent to your email.</p>
            </div>
            
            <form action="<?= htmlspecialchars(BASE_URL) ?>/index.php?action=verify-otp" method="POST" class="space-y-6">
                <?= csrf_field() ?>
                
                <div>
                    <label for="otp" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5 text-center">4-Digit Code</label>
                    <div class="relative max-w-xs mx-auto">
                        <input type="text" name="otp" id="otp" required maxlength="4" pattern="\d{4}" class="block w-full text-center text-3xl tracking-[1em] rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 focus:ring-2 py-4 outline-none transition-all placeholder-slate-300 dark:placeholder-slate-700" placeholder="••••">
                    </div>
                </div>
        ...
        <div class="px-8 py-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-700/50 text-center flex items-center justify-between">
            <a href="<?= htmlspecialchars(BASE_URL) ?>/views/auth/forgot-password.php" class="text-xs text-indigo-600 dark:text-indigo-400 font-semibold hover:underline flex items-center gap-1">
