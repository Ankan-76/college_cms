<?php
// views/faculty/edit-profile.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        redirect('/views/faculty/edit-profile.php');
    }
    ...
    if ($action === 'update_details') {
        ...
        if (empty($name)) {
            ...
            $stmt = $db->prepare("UPDATE teachers SET name = ?, phone = ? WHERE id = ?");
        ...
        if (empty($currentPassword) || empty($newPassword) || empty($confirmPassword)) {
            ...
            if (password_verify($currentPassword, $hash)) {
                ...
                $updateStmt = $db->prepare("UPDATE teachers SET password_hash = ? WHERE id = ?");
        ...
        if (isset($_FILES['profile_pic']) && $_FILES['profile_pic']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($file['type'], $allowedTypes)) {
                ...
                if (move_uploaded_file($file['tmp_name'], $destination)) {
                    ...
                    $stmt = $db->prepare("SELECT profile_pic FROM teachers WHERE id = ?");
                    $stmt->execute([$userId]);
                    $oldPic = $stmt->fetchColumn();

                    // Update DB
                    $updateStmt = $db->prepare("UPDATE teachers SET profile_pic = ? WHERE id = ?");
...
<main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 bg-slate-50 dark:bg-slate-900 transition-colors duration-200">
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            ...
            <a href="<?= BASE_URL ?>/views/faculty/view-profile.php" class="inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-1 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 text-center">
                    ...
                    <p class="text-indigo-600 dark:text-indigo-400 font-medium text-sm mb-4"><?= htmlspecialchars($user['designation'] ?? 'FACULTY') ?></p>
                    
                    <form action="<?= BASE_URL ?>/views/faculty/edit-profile.php" method="POST" enctype="multipart/form-data" class="mt-4">
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-2">JPG, PNG, GIF (Max 2MB). Auto-uploads on selection.</p>
                    </form>
                </div>
            </div>
            ...
            <div class="md:col-span-2 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <form action="<?= BASE_URL ?>/views/faculty/edit-profile.php" method="POST" class="p-6 space-y-4">
                        <input type="hidden" name="csrf_token" value="<?= generate_csrf_token() ?>">
