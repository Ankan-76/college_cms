<?php
// views/faculty/my_feedbacks.php — Faculty Feedback Management
require_once __DIR__ . '/../../config/app.php';
require_once __DIR__ . '/../../includes/auth_middleware.php';

require_role('FACULTY');
$pageTitle = 'My Feedbacks | Faculty Portal';
...
$allFeedbacks = $controller->getUserFeedbacks($facultyId, 'FACULTY');
...
<main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 bg-slate-50 dark:bg-slate-900 transition-colors duration-200">
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        <!-- Top Header Banner -->
        <div class="bg-gradient-to-r from-emerald-600 via-teal-600 to-indigo-700 rounded-2xl p-6 sm:p-8 text-white shadow-xl relative overflow-hidden">
            ...
            <div class="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-white/20 text-white backdrop-blur-sm mb-3">
                        <i data-lucide="message-square-heart" class="w-3.5 h-3.5"></i> Faculty Governance & Voice
                    </span>
                    <h1 class="text-2xl sm:text-3xl font-black tracking-tight">Faculty Feedback Submissions</h1>
                    <p class="text-emerald-100 text-sm mt-1 max-w-xl">
                        Review academic suggestions, infrastructure reports, and administrative responses from the college leadership.
                    </p>
                </div>
                <div class="flex-shrink-0">
                    <a href="<?= $base ?>/feedback.php" class="inline-flex items-center gap-2 bg-white text-emerald-700 hover:bg-emerald-50 font-bold px-5 py-2.5 rounded-xl shadow-lg transition-all hover:-translate-y-0.5 text-xs sm:text-sm">
        ...
        <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-2 sm:p-3 rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-sm">
            ...
            <form method="GET" action="" class="relative shrink-0">
                ...
                <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Search subject or keyword..."
                    class="w-full md:w-64 pl-9 pr-8 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 text-slate-900 dark:text-white text-xs focus:ring-2 focus:ring-emerald-500 font-medium">
                <?php if (!empty($search)): ?>
        ...
        <?php if (empty($feedbacks)): ?>
            <div class="bg-white dark:bg-slate-800 rounded-3xl p-10 sm:p-12 text-center border border-slate-200 dark:border-slate-700/80 shadow-sm">
                <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-emerald-50 dark:bg-emerald-900/30 text-emerald-500 mb-4">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto">
                    <?= !empty($search) || !empty($statusFilter) ? 'No feedback submissions match your selected filter criteria.' : "You haven't submitted any feedback yet. Your proposals help elevate Greenfield College!" ?>
                </p>
                <div class="mt-6 flex flex-wrap items-center justify-center gap-3">
                    ...
                    <?php endif; ?>
                    <a href="<?= $base ?>/feedback.php" class="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold shadow-md shadow-emerald-500/25 transition-all">
            ...
            <div class="space-y-4">
                <?php foreach ($feedbacks as $fb): 
                    ...
                    if ($fb['status'] === 'NEW') {
                        ...
                        $statusBadge = '<span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800/40"><i data-lucide="check" class="w-3.5 h-3.5"></i> Reviewed by Leadership</span>';
                    } else {
                ...
                ?>
                    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/80 p-5 sm:p-6 transition-all hover:shadow-md">
                        ...
                        <?php if (!empty($fb['admin_notes'])): ?>
                            <div class="mt-4 p-4 rounded-xl bg-emerald-50/70 dark:bg-emerald-950/40 border border-emerald-200/60 dark:border-emerald-800/50 space-y-1">
                                <div class="flex items-center gap-2 text-xs font-bold text-emerald-700 dark:text-emerald-300 uppercase tracking-wider">
                                    <i data-lucide="shield-check" class="w-4 h-4 text-emerald-500"></i> Response from Administration
                                </div>
