<?php
// views/faculty/my_leaves.php
require_once __DIR__ . '/../../config/app.php';
require_once __DIR__ . '/../../includes/auth_middleware.php';

require_role('FACULTY');
$pageTitle = 'My Leaves | Faculty Portal';
...
$leaves = $controller->getMyLeaves((int) $_SESSION['user_id'], 'FACULTY', $statusFilter);
$base = defined('BASE_URL') ? BASE_URL : '/college_cms';

// Count by status for tabs
$allLeaves = $controller->getMyLeaves((int) $_SESSION['user_id'], 'FACULTY');
...
<main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 bg-slate-50 dark:bg-slate-900 transition-colors duration-200">
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
                    <i data-lucide="calendar-check" class="w-6 h-6 text-emerald-500"></i> My Leave Requests
                </h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Track and manage all your submitted leave applications.</p>
            </div>
            <a href="<?= $base ?>/views/faculty/apply_leave.php" class="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-bold shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 transition-all hover:-translate-y-0.5">
        ...
        <div class="flex items-center gap-1 bg-white dark:bg-slate-800 p-1.5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm w-fit">
            ...
            ?>
                <a href="<?= $base ?>/views/faculty/my_leaves.php<?= $tab['value'] ? '?status=' . $tab['value'] : '' ?>" 
        ...
        <?php if (empty($leaves)): ?>
            <div class="bg-white dark:bg-slate-800 rounded-xl p-10 text-center border border-slate-200 dark:border-slate-700 shadow-sm">
                ...
                <a href="<?= $base ?>/views/faculty/apply_leave.php" class="inline-flex items-center gap-2 mt-4 px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-semibold transition-colors">
        ...
        <?php else: ?>
            <div class="space-y-4">
                ...
                ?>
                    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden hover:shadow-md transition-all duration-300">
