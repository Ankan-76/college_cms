<?php
// views/faculty/view-profile.php
...
$stmt = $db->prepare("
    SELECT t.name, t.email, t.phone, t.status, t.created_at, t.profile_pic,
           t.designation, t.qualification, GROUP_CONCAT(d.dept_name ORDER BY d.dept_name ASC SEPARATOR ', ') as dept_names
    FROM teachers t
...
<main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 bg-slate-50 dark:bg-slate-900 transition-colors duration-200">
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            ...
            <a href="<?= BASE_URL ?>/views/faculty/edit-profile.php" class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors shadow-sm focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-slate-900">
            ...
            <div class="px-6 sm:px-8 pb-8">
                ...
                <div class="flex flex-col sm:flex-row items-center sm:items-end -mt-16 sm:-mt-20 gap-6 mb-8">
                    <div class="relative group/avatar">
                        ...
                        <?php endif; ?>
                        <a href="<?= BASE_URL ?>/views/faculty/edit-profile.php" class="absolute inset-0 bg-slate-900/60 rounded-full opacity-0 group-hover/avatar:opacity-100 transition-opacity flex flex-col items-center justify-center text-white backdrop-blur-sm m-1">
                    ...
                    <div class="flex-1 text-center sm:text-left mb-2">
                        ...
                        <p class="text-indigo-600 dark:text-indigo-400 font-medium tracking-wide mt-1 uppercase"><?= htmlspecialchars($user['designation'] ?? 'Faculty') ?></p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 border-t border-slate-100 dark:border-slate-700/50">
                    ...
                    <div>
                        <h3 class="text-sm font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-4">Academic & Account</h3>
                        <div class="space-y-4">
                            <?php if(!empty($user['dept_names'])): ?>
                            <div class="flex items-start gap-3">
                                <div class="p-2 rounded-lg bg-slate-50 dark:bg-slate-700/50 text-slate-400 dark:text-slate-300">
                                    <i data-lucide="building" class="w-5 h-5"></i>
                                </div>
                                <div>
                                    <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">Department(s)</p>
                                    <p class="text-sm font-medium text-slate-800 dark:text-slate-200"><?= htmlspecialchars($user['dept_names']) ?></p>
                                </div>
                            </div>
                            <?php endif; ?>
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">Qualification</p>
                                    <p class="text-sm font-medium text-slate-800 dark:text-slate-200"><?= htmlspecialchars($user['qualification']) ?></p>
                                </div>
                            </div>
                            <?php endif; ?>
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">Joined Date</p>
