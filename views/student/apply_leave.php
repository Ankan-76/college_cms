<?php
// views/student/apply_leave.php
require_once __DIR__ . '/../../config/app.php';
require_once __DIR__ . '/../../includes/auth_middleware.php';

require_role('STUDENT');
$pageTitle = 'Apply for Leave | Student Portal';
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
                    <i data-lucide="calendar-off" class="w-6 h-6 text-indigo-500"></i> Apply for Leave
                </h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Submit a leave request for approval by the administration.</p>
            </div>
            <a href="<?= $base ?>/views/student/my_leaves.php" class="inline-flex items-center gap-2 text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors">
            ...
            <form action="<?= $base ?>/controllers/process_leave.php" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
                        <i data-lucide="tag" class="w-4 h-4 inline-block mr-1 text-indigo-500"></i> Leave Type
                    </label>
                ...
                <div id="custom-subject-container" class="hidden">
                    <label for="custom_subject" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
                        <i data-lucide="edit-3" class="w-4 h-4 inline-block mr-1 text-indigo-500"></i> Subject / Reason Type
                    </label>
                    <input type="text" id="custom_subject" name="custom_subject" minlength="3" maxlength="255"
                        placeholder="E.g., Family Emergency, Wedding"
                        class="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm placeholder:text-slate-400">
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" id="start_date" name="start_date" required
                            min="<?= date('Y-m-d') ?>"
                            class="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm font-medium">
                    </div>
                    <div>
                ...
                <div id="duration-preview" class="hidden">
                    <div class="flex items-center gap-3 p-3 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/30">
                        <i data-lucide="clock" class="w-5 h-5 text-indigo-500"></i>
                        <span class="text-sm font-semibold text-indigo-700 dark:text-indigo-300">
                            Duration: <span id="duration-days" class="text-indigo-600 dark:text-indigo-400">0</span> day(s)
                        </span>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="reason" name="reason" rows="4" required minlength="10"
                        ...
                        class="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm resize-none placeholder:text-slate-400"></textarea>
                ...
                <div>
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 dark:border-slate-600 border-dashed rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors relative group">
                        <div class="space-y-1 text-center">
                            <i data-lucide="upload-cloud" class="mx-auto h-12 w-12 text-slate-400 group-hover:text-indigo-500 transition-colors"></i>
                            <div class="flex text-sm text-slate-600 dark:text-slate-400 justify-center">
                                <label for="supporting_docs" class="relative cursor-pointer bg-white dark:bg-slate-700 rounded-md font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    <a href="<?= $base ?>/views/student/my_leaves.php" class="px-5 py-2.5 rounded-xl text-sm font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                        Cancel
                    </a>
                    <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 transition-all hover:-translate-y-0.5">
