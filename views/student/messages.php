<?php
// views/student/messages.php
require_once __DIR__ . '/../../config/app.php';
require_once __DIR__ . '/../../includes/auth_middleware.php';

require_role('STUDENT');
$pageTitle = 'Messages | Student Portal';
...
$conversations = $controller->getConversations($userId, 'STUDENT');
$contacts = $controller->getContactableUsers($userId, 'STUDENT');
...
if ($isBroadcast) {
    $activeMessages = $controller->getAdminBroadcastsForUser($userId, 'STUDENT');
} elseif ($activeConvId) {
    $activeMessages = $controller->getMessages((int)$activeConvId, $userId, 'STUDENT');
    $activeConversation = $controller->getConversation((int)$activeConvId, $userId, 'STUDENT');
    $pinnedMessages = $controller->getPinnedMessages((int)$activeConvId, $userId, 'STUDENT');
            ...
            <div class="flex items-center justify-between max-w-full">
                <div>
                    <h1 class="text-xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
                        <i data-lucide="message-circle" class="w-5 h-5 text-indigo-500"></i> Messages
                    </h1>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Chat with your classmates and course faculty members.</p>
                </div>
            ...
            <div id="conv-sidebar" class="w-80 flex-shrink-0 border-r border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex flex-col <?= $activeConvId ? 'hidden md:flex' : 'flex' ?>">
                ...
                <div class="p-3 border-b border-slate-200 dark:border-slate-700 space-y-2.5">
                    ...
                    <button onclick="document.getElementById('new-conv-modal').classList.remove('hidden')" 
                        class="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 transition-all">
                        <i data-lucide="plus" class="w-4 h-4"></i> New Conversation
                    </button>

                    <!-- Filter Tabs: All, Unread, Faculty, Classmates -->
                    <div class="flex items-center p-1 bg-slate-100 dark:bg-slate-700/60 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300" id="conv-tabs">
                        <button type="button" onclick="filterConversations('all', this)" class="conv-tab-btn flex-1 py-1.5 text-center rounded-lg transition-all bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-sm">
                            All
                        </button>
                        ...
                        <button type="button" onclick="filterConversations('classmates', this)" class="conv-tab-btn flex-1 py-1.5 text-center rounded-lg transition-all hover:text-slate-900 dark:hover:text-white">
                            Classmates
                        </button>
                    </div>
                </div>
                ...
                <div class="flex-1 overflow-y-auto" id="conv-list">
                    ...
                    <a href="<?= $base ?>/views/student/messages.php?conv=broadcast" 
                       data-category="broadcast"
                       data-unread="false"
                       data-search-text="college admin official announcements broadcast"
                       class="conv-item flex items-center gap-3 px-4 py-3 border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors <?= $isBroadcast ? 'bg-indigo-50 dark:bg-indigo-900/20 border-l-4 border-l-indigo-500' : '' ?>">
                    ...
                    <?php if (empty($conversations)): ?>
                        <div class="p-6 text-center" id="empty-conv-notice">
                            ...
                            <p class="text-xs text-slate-400 mt-1">Click above to start a chat with a faculty or classmate</p>
                        </div>
                    <?php else: ?>
                        <?php foreach ($conversations as $conv): 
                            $isActive = $activeConvId === (int) $conv['id'];
                            $contactRole = $conv['contact_role'] ?? 'FACULTY';
                            $category = ($contactRole === 'STUDENT') ? 'student' : 'faculty';
                            $unreadCount = (int) ($conv['unread_count'] ?? 0);
                            $contactName = $conv['contact_name'] ?? 'Contact';
                            $avatarColor = ($contactRole === 'STUDENT') ? '06b6d4' : '4f46e5';

                            $contactPic = !empty($conv['contact_pic']) 
                                ? $base . '/uploads/profiles/' . htmlspecialchars($conv['contact_pic'])
                                : 'https://ui-avatars.com/api/?name=' . urlencode($contactName) . '&background=' . $avatarColor . '&color=fff&bold=true';
                        ?>
                            <a href="<?= $base ?>/views/student/messages.php?conv=<?= $conv['id'] ?>" 
                               data-category="<?= $category ?>"
                               data-unread="<?= $unreadCount > 0 ? 'true' : 'false' ?>"
                               data-search-text="<?= htmlspecialchars(strtolower($contactName . ' ' . ($conv['last_message'] ?? '') . ' ' . ($conv['contact_roll'] ?? ''))) ?>"
                               class="conv-item flex items-center gap-3 px-4 py-3 border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors <?= $isActive ? 'bg-indigo-50 dark:bg-indigo-900/20 border-l-4 border-l-indigo-500' : '' ?>">
                                <div class="relative flex-shrink-0">
                                    <img src="<?= $contactPic ?>" alt="" class="w-10 h-10 rounded-full object-cover border-2 border-slate-200 dark:border-slate-600">
                                    <?php if ($contactRole === 'STUDENT'): ?>
                                        <span class="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-cyan-500 border-2 border-white dark:border-slate-800" title="Classmate"></span>
                                    <?php endif; ?>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between">
                                        <span class="text-sm font-bold text-slate-900 dark:text-white truncate"><?= htmlspecialchars($contactName) ?></span>
                                        <?php if ($unreadCount > 0): ?>
                                            <span class="inline-flex items-center justify-center w-5 h-5 text-[10px] font-bold text-white bg-indigo-600 rounded-full flex-shrink-0 animate-pulse"><?= $unreadCount ?></span>
                                        <?php endif; ?>
                                    </div>
                                    <div class="flex items-center justify-between mt-0.5">
                                        <p class="text-xs text-slate-500 dark:text-slate-400 truncate"><?= htmlspecialchars($conv['last_message'] ?? 'No messages yet') ?></p>
                                        <span class="text-[9px] px-1 py-0.2 rounded font-semibold <?= $contactRole === 'STUDENT' ? 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400' : 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' ?>">
                                            <?= $contactRole === 'STUDENT' ? 'Peer' : 'Faculty' ?>
                                        </span>
                                    </div>
                                </div>
                            </a>
                ...
                <?php if (!$activeConvId): ?>
                    ...
                    <div class="flex-1 flex items-center justify-center p-8">
                        <div class="text-center">
                            <div class="w-20 h-20 mx-auto mb-4 rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 flex items-center justify-center">
                                <i data-lucide="message-circle" class="w-10 h-10 text-indigo-400 dark:text-indigo-600"></i>
                            </div>
                            <h3 class="text-lg font-bold text-slate-900 dark:text-white">Select a conversation</h3>
                            <p class="text-sm text-slate-500 mt-1">Choose a classmate or faculty conversation from the sidebar, or start a new message.</p>
                        </div>
                    </div>
                <?php else: ?>
                    <!-- Chat Header -->
                    <div class="relative px-4 py-3 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex items-center justify-between flex-shrink-0">
                        <div class="flex items-center gap-3 min-w-0">
                            <a href="<?= $base ?>/views/student/messages.php" class="md:hidden text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 p-1 rounded-lg">
                            ...
                            <?php else: ?>
                                ...
                                $chatSubtitle = $activeConversation['contact_subtitle'] ?? 'Classmate';
                                $isPeerChat = ($activeConversation['contact_role'] ?? '') === 'STUDENT';
                                $avatarColor = $isPeerChat ? '06b6d4' : '4f46e5';
                                ...
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider <?= $isPeerChat ? 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400' : 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' ?>">
                                            <?= $isPeerChat ? 'Classmate' : 'Faculty' ?>
                                        </span>
                                    </div>
                                    <p class="text-[11px] text-slate-500 dark:text-slate-400 font-medium"><?= htmlspecialchars($chatSubtitle) ?></p>
                                </div>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="flex-1 overflow-y-auto p-4 space-y-3" id="messages-container">
                        <?php if (empty($activeMessages)): ?>
                            <div class="text-center py-10" id="no-messages-placeholder">
                                ...
                                <p class="text-sm text-slate-400">No messages yet. Send a greeting or discussion question!</p>
                            </div>
                        <?php else: ?>
                            <?php foreach ($activeMessages as $msg): 
                                $isMine = !$isBroadcast && ($msg['sender_type'] === 'STUDENT' && (int) $msg['sender_id'] === $userId);
                            ...
                            ?>
                                <div class="message-wrapper flex items-start gap-2 <?= $isMine ? 'justify-end' : 'justify-start' ?>" id="msg-wrapper-<?= $msg['id'] ?>" data-msg-id="<?= $msg['id'] ?>" data-mine="<?= $isMine ? 'true' : 'false' ?>">
                                    ...
                                    <div class="select-checkbox-col hidden pt-2.5">
                                        <input type="checkbox" class="msg-select-check w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500 border-slate-300 dark:border-slate-600" value="<?= $msg['id'] ?>" onchange="updateSelectedCount()">
                                    </div>
                                    ...
                                    <div class="relative group max-w-[78%] sm:max-w-[70%]">
                                        <div class="message-bubble relative rounded-2xl px-4 py-2.5 shadow-sm transition-all <?= $isMine 
                                            ? ($isUnsent ? 'bg-slate-200 dark:bg-slate-700 text-slate-500 italic rounded-br-md border border-slate-300 dark:border-slate-600' : 'bg-indigo-600 text-white rounded-br-md')
                                            ...
                                            <?php if (!empty($msg['reply_to_content'])): ?>
                                                <div class="mb-2 p-2 rounded-lg text-xs cursor-pointer border-l-4 <?= $isMine ? 'bg-indigo-700/60 border-indigo-300 text-indigo-100' : 'bg-slate-100 dark:bg-slate-800/80 border-indigo-500 text-slate-700 dark:text-slate-300' ?>" onclick="scrollToMessage(<?= (int)$msg['reply_to_id'] ?>)">
                                            ...
                                            <!-- Timestamp & Status -->
                                            <div class="flex items-center justify-end gap-1 text-[10px] mt-1 <?= $isMine ? ($isUnsent ? 'text-slate-400' : 'text-indigo-200') : 'text-slate-400' ?>">
                                        ...
                                        <div class="reaction-badges flex flex-wrap gap-1 mt-1 <?= $isMine ? 'justify-end' : 'justify-start' ?>" id="rx-badges-<?= $msg['id'] ?>">
                                            <?php if (!empty($msg['reactions'])): ?>
                                                ...
                                                ?>
                                                    <button type="button" onclick="toggleReaction(<?= $msg['id'] ?>, '<?= $emoji ?>')" class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[11px] font-medium border transition-all <?= $userReacted ? 'bg-indigo-100 border-indigo-300 dark:bg-indigo-900/50 dark:border-indigo-700 text-indigo-700 dark:text-indigo-300' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-100' ?>">
                                                        <span><?= $emoji ?></span>
                    ...
                    <div id="reply-preview-bar" class="hidden px-4 py-2 bg-indigo-50 dark:bg-indigo-900/30 border-t border-indigo-200 dark:border-indigo-800 flex items-center justify-between text-xs text-indigo-900 dark:text-indigo-200">
                        <div class="flex items-center gap-2 min-w-0">
                            <i data-lucide="reply" class="w-4 h-4 text-indigo-600 dark:text-indigo-400 flex-shrink-0"></i>
                        ...
                        <div class="border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-3 flex-shrink-0" id="msg-composer-wrapper">
                            <form id="msg-form" class="flex items-end gap-2">
                                ...
                                <textarea id="msg-input" name="content" rows="1" 
                                    ...
                                    class="flex-1 px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-slate-100 text-sm resize-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all placeholder:text-slate-400 max-h-32"
                                    onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();sendMessage();}"></textarea>
                                <button type="button" onclick="sendMessage()" class="flex-shrink-0 w-10 h-10 flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-lg shadow-indigo-500/25 transition-all hover:scale-105">
...
<div id="clear-chat-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-sm p-5 z-10">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 text-center mt-1.5 leading-relaxed">
            This will clear messages in this conversation <strong>for your account only</strong>. The other person will still keep their conversation history.
        </p>
...
<!-- New Conversation Modal (With Classmates & Faculty Tabs) -->
    ...
    <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-md max-h-[82vh] flex flex-col overflow-hidden z-10">
        ...
        <div class="p-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
            <h3 class="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <i data-lucide="user-plus" class="w-5 h-5 text-indigo-500"></i> New Conversation
            </h3>
        ...
        <div class="p-3 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 space-y-2">
            <div class="relative">
                ...
                <input type="text" id="contact-search-input" placeholder="Search by name or roll number..." oninput="filterModalContacts(this.value)" class="w-full pl-9 pr-3 py-1.5 text-xs rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 placeholder:text-slate-400">
            </div>
            
            <?php 
            $classmatesCount = count(array_filter($contacts, fn($c) => ($c['category'] ?? '') === 'Classmate'));
            ...
            <div class="flex items-center gap-1.5 text-xs font-semibold" id="modal-filter-pills">
                <button type="button" onclick="switchModalTab('all', this)" class="modal-tab-btn px-2.5 py-1 rounded-lg bg-indigo-600 text-white shadow-xs">
                    All (<?= count($contacts) ?>)
                </button>
                <button type="button" onclick="switchModalTab('Classmate', this)" class="modal-tab-btn px-2.5 py-1 rounded-lg bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-300">
        ...
        <div class="p-4 overflow-y-auto flex-1 space-y-2" id="modal-contacts-list">
            ...
            <?php else: ?>
                <?php foreach ($contacts as $contact): 
                    $isPeer = ($contact['category'] ?? '') === 'Classmate';
                    $avatarColor = $isPeer ? '06b6d4' : '4f46e5';
                ...
                ?>
                    <form action="<?= $base ?>/controllers/process_message.php" method="POST" class="modal-contact-item" data-category="<?= htmlspecialchars($contact['category'] ?? '') ?>" data-search-text="<?= htmlspecialchars(strtolower($contact['name'] . ' ' . ($contact['roll_number'] ?? '') . ' ' . ($contact['email'] ?? ''))) ?>">
                        <input type="hidden" name="csrf_token" value="<?= $csrfToken ?>">
                        ...
                        <button type="submit" class="w-full flex items-center justify-between p-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-100 dark:border-slate-700 hover:border-slate-200 dark:hover:border-slate-600 transition-all text-left group">
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <p class="text-xs font-bold text-slate-900 dark:text-white truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors"><?= htmlspecialchars($contact['name']) ?></p>
                                    <p class="text-[11px] text-slate-500 dark:text-slate-400 truncate"><?= htmlspecialchars($contact['subtitle'] ?? $contact['email']) ?></p>
                                </div>
                            </div>
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider <?= $isPeer ? 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400' : 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' ?>">
...
function filterConversations(tab, btn) {
    ...
    document.querySelectorAll('.conv-tab-btn').forEach(b => {
        b.classList.remove('bg-white', 'dark:bg-slate-800', 'text-indigo-600', 'dark:text-indigo-400', 'shadow-sm');
...
function applySidebarFilters() {
    ...
    items.forEach(item => {
        ...
        } else if (currentConvTab === 'classmates') {
                ...
                html += `
                    <div class="p-3 bg-slate-50 dark:bg-slate-700/60 rounded-xl border border-slate-200 dark:border-slate-600 text-xs space-y-1.5">
                        ...
                        <div class="flex items-center justify-between pt-1 border-t border-slate-200/50 dark:border-slate-600/50">
                            <a href="${BASE_URL}/views/student/messages.php?conv=${s.conversation_id}#msg-wrapper-${s.message_id}" onclick="closeStarredDrawer()" class="text-indigo-600 dark:text-indigo-400 font-semibold hover:underline flex items-center gap-1">
                                Jump to chat &rarr;
                            </a>
...
function renderMessages(messages, pinned, forceScroll) {
    ...
    messages.forEach(msg => {
        const isMine = msg.sender_type === 'STUDENT' && parseInt(msg.sender_id) === CURRENT_USER_ID;
        ...
        if (msg.reply_to_content) {
            quoteHtml = `
                <div class="mb-2 p-2 rounded-lg text-xs cursor-pointer border-l-4 ${isMine ? 'bg-indigo-700/60 border-indigo-300 text-indigo-100' : 'bg-slate-100 dark:bg-slate-800/80 border-indigo-500 text-slate-700 dark:text-slate-300'}" onclick="scrollToMessage(${msg.reply_to_id})">
