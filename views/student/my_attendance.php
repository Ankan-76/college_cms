    ...
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-5 text-center">
                <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Total Classes</p>
                <p class="text-3xl font-black text-slate-900 dark:text-white"><?= $overallTotal ?></p>
            </div>
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-5 text-center">
                <p class="text-xs font-bold text-emerald-500 uppercase tracking-wider mb-2">Present</p>
                <p class="text-3xl font-black text-emerald-600 dark:text-emerald-400"><?= $overallPresent ?></p>
            </div>
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-5 text-center">
                <p class="text-xs font-bold text-rose-500 uppercase tracking-wider mb-2">Absent</p>
                <p class="text-3xl font-black text-rose-600 dark:text-rose-400"><?= $overallAbsent ?></p>
            </div>
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-5 text-center">
                <?php $overallColor = $overallPct >= 75 ? 'emerald' : ($overallPct >= 50 ? 'amber' : 'rose'); ?>
                <p class="text-xs font-bold text-<?= $overallColor ?>-500 uppercase tracking-wider mb-2">Overall</p>
                <p class="text-3xl font-black text-<?= $overallColor ?>-600 dark:text-<?= $overallColor ?>-400"><?= $overallPct ?>%</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            ?>
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 hover:shadow-md transition-all">
                <div class="flex items-start justify-between mb-4">
                    <div>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-indigo-100 text-indigo-800 dark:bg-indigo-900/40 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/50 mb-2">
                            <?= htmlspecialchars($course['course_code']) ?>
                        </span>
                        <h3 class="text-base font-bold text-slate-900 dark:text-white"><?= htmlspecialchars($course['course_name']) ?></h3>
                        <p class="text-xs text-slate-500 mt-0.5"><?= $course['credits'] ?> Credits</p>
                    </div>
                    <div class="text-right">
                        <span class="text-2xl font-black text-<?= $color ?>-600 dark:text-<?= $color ?>-400"><?= $pct ?>%</span>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between text-xs font-medium text-slate-500">
                    <span class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-emerald-500"></span> Present: <?= $present ?></span>
                    <span class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-amber-500"></span> Late: <?= $late ?></span>
                    <span class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-rose-500"></span> Absent: <?= $absent ?></span>
